...
class RobocodeEnv:
    ...
    def process_observation(self, game_state):
        return torch.tensor([
            game_state.robotState.heading / 360.0,  # Normalize heading
            game_state.enemyState.bearing / 360.0,  # Normalize enemy bearing
            game_state.robotState.gunHeading / 360.0,  # Normalize gun heading
    ...
    def calculate_reward(self, previous_state, previous_action, current_state):
        ...
        # Calculate the gun bearing for both previous and current states
        previous_gun_bearing = self.calculate_bearing_from_gun(
            previous_state.robotState.heading,
            previous_state.enemyState.bearing,
            previous_state.robotState.gunHeading
        )

        current_gun_bearing = self.calculate_bearing_from_gun(
            current_state.robotState.heading,
            current_state.enemyState.bearing,
            current_state.robotState.gunHeading
        )
        ...
        gun_turn_reward = self._calculate_gun_turn_reward(previous_gun_bearing, current_gun_bearing)
